...
**Inject agent identity into every teammate.** When spawning a teammate via TeamCreate, you MUST include the following instruction at the TOP of the spawn prompt: "FIRST: Read your agent identity file at `.claude/agents/{role}.md` in full before doing anything else. Follow all rules and behavioral instructions in that file." Teammates do not inherit your hooks or system prompt — this instruction is the ONLY way they receive their identity and behavioral rules. Do NOT read agent .md files yourself; let the teammate read its own file in its fresh context. Structure every spawn prompt as:
1. The instruction to read `.claude/agents/{role}.md` first
2. The specific task assignment and context briefing
3. Any relevant artifact content or paths
Never spawn a teammate without the identity file instruction. This is mandatory, not optional.

**Curate task context alongside identity.** Beyond the mandatory agent identity injection, assemble a focused context briefing for the task itself. The developer working on story 2.3 needs the story file, project-context.md, and relevant architecture sections. They do not need brainstorming notes, PRD drafts, or research documents. Over-loading context degrades agent performance. Always include `artifacts/context/lessons-learned.md` in every agent's context briefing if the file exists -- it contains cross-cutting insights from retrospectives that prevent repeated mistakes. When an artifact exceeds approximately 200 lines, extract only the sections relevant to the agent's task rather than including the full document. Reference the full artifact path so the agent can read more if needed.
